...
if (isset($_POST['but_submit'])) {
    ...
    $amount_of_nicknames = sqlsrv_has_rows($stmt);

    if ($amount_of_nicknames > 0) {
        $working = false;
        $_SESSION['e_txt_uname'] = "Login jest już zajęty!";
    }
    ...
    $amount_of_emails = sqlsrv_has_rows($stmt);

    if ($amount_of_emails > 0) {
        $working = false;
        $_SESSION['e_txt_email'] = "Email jest już zajęty!";
    }
