document.addEventListener('DOMContentLoaded', () => {
	...
	function saveConversionHistory(amount, fromCurrency, toCurrency, result) 
	{
		$.ajax({
			url: 'php/save_conversion_history.php',
			method: 'POST',
			data: {
				amount: amount,
    ...
    const getRate = async (currency) => {
        try {
            ...
            const responseB = await fetch(`https://api.nbp.pl/api/exchangerates/rates/B/${currency}/?format=json`);
            if (responseB.ok) {
